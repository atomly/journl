...
const zModelUsageEvent = z.object({
...
type LanguageModelUsedEvent = z.infer<typeof zModelUsageEvent>;
...
export async function onModelUsage(
  event: LanguageModelUsedEvent,
): Promise<UsageAggregationResult> {
  "use workflow";

  const payload = await validateLanguageModelUsageRecordedStep(event);
...
async function validateLanguageModelUsageRecordedStep(
  event: LanguageModelUsedEvent,
): Promise<LanguageModelUsedEvent> {
  "use step";

  return zModelUsageEvent.parse(event);
}
